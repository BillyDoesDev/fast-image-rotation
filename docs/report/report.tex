...
\begin{algorithm}[h]
  \caption{Algorithm to compute the rotated image, based on input parameters as defined by the angle of rotation, $\alpha$}
  \label{algo:rotate_matrix}
  \DontPrintSemicolon
  \SetKwInOut{algor}{ROTATE\_MATRIX($d, IV, k$)}
  % \SetKwInOut{Output}{Output}
  % \SetKwFunction{bin}{BINARY}
  % \SetKwFunction{bpd}{BIT-PLANE\_DECOMPOSITION}
  % \algor{}%{$(I,m,n,p)$}
  % Divide the DNA sequence $d$ into non-overlapping blocks of 64 nucleotides each to obtain $D_0, D_1,...,D_n$\;
  % If the length of $D_n <64$ pad it with $\mathtt{A}$'s\;
  % $D_0' = $ \textbf{DNA-XOR($IV, D_0$)}\;
  % $C_0 = $\textbf{DNA-AES($D_0',k$)}\;%\tcp*{$k$ is the encryption key}
  % \For{$i = 1$ \KwTo $n$}{
  %     $D_i' = $ \textbf{DNA-XOR($C_{i-1}, D_i$)}\;
  %     $C_i = $ \textbf{DNA-AES($D_i',k$)}\;
  % }
  % $C = C_0+C_1+...+C_n$\;
  % \Return $C$\;
\end{algorithm}
