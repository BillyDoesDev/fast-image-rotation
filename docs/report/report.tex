...
Apart from this, depending on $\alpha$, the pixel processing order changes - i.e., whether the last row or column vector of pixels get processed first, or last. Since it has been found out that these values are very sensitive to $\alpha$, the specific details are broken down in the rotation algorithm described later on in this paper.
...
The rotation function is then invoked, with each of the parameters updating depending on the value of $\alpha$, as discussed above. The detailed function call is described in the algorithm for the driver function below:
